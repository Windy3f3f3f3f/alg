#include <bits/stdc++.h>

using namespace std;
const int N = 1e5+10;
const int M = 2e6+10;
int mod;
int n,m;

int h[N], hs[N], e[M], ne[M], idx;
int dfn[N], low[N], timestamp;
int stk[N], top;
bool in_stk[N];
int id[N], scc_cnt, scc_size[N];
int f[N], g[N];
void add(int h[], int a, int b){
    e[idx]=b, ne[idx]=h[a], h[a]=idx++; //记住，这里h一定要初始化
}

void tarjan(int u){
    dfn[u] = low[u] = ++timestamp;
    stk[++top] = u, in_stk[u] = true;
    for(int i=h[u];i!=-1;i=ne[i]){
        int j = e[i];
        if(!dfn[j]){//如果没有遍历过，继续tarjan
            tarjan(j);
            low[u] = min(low[u],low[j]);//可能存在更高的反向边、跨边
        }else if(in_stk[j]){
            //如果是反向边
            low[u] = min(low[u],dfn[j]);//直接用j的时间戳更新u
        }
    }
    // 所以当遍历完u的所有能到的点后 发现u最高能到的点是自己
    // 1 则u为强连通分量中的最高点,则以u为起点往下把该强连通分量所有节点都找出来
    // 2 要么它就没有环,就是一个正常的往下的点
    if(dfn[u]==low[u]){
        int y;
        ++scc_cnt;
        do{
            y = stk[top--];
            in_stk[y]=false;
            id[y]=scc_cnt;
            scc_size[scc_cnt]++;
        }while(y!=u); //弹出，包括弹他自己
    }
}

int main(){
    cin>>n>>m>>mod;
    memset(h, -1, sizeof h);
    memset(hs, -1, sizeof hs);
    while(m--){
        int a, b;
        scanf("%d%d",&a,&b);
        add(h, a, b);
    }
    //求一遍tarjan
    for(int i=1;i<=n;i++){
        if(!dfn[i])
            tarjan(i);
    }
    unordered_set<long long> S;    // (u, v) -> u * 1000000 + v
    //缩点
    for(int a=1;a<=n;a++){
        for(int idx=h[a];idx!=-1;idx=ne[idx]){
            int b = e[idx];
            int aa=id[a], bb=id[b];
            if(aa!=bb){//注意到，这里后面要求最长路，而且是用拓扑图的dp来求，不允许有重复边
                long long hash = aa * 1000000ll + bb;
                if(aa!=bb && !S.count(hash)){
                    add(hs, aa, bb);
                    S.insert(hash);
                }
            }
        }
    }
/*
dp
f为到i的最长路，并求出该路径节点数
g： 从 入度为0的点开始 到 i的最长路的数目
i为拓扑序

f[拓扑排序起点] = size[拓扑排序起点]
g[拓扑排序起点] = 1
for i: 拓扑序
    for j: j能到i
        if(f[j]+Size[i]>f[i]){
            f[i] = max(f[j]+Size[o])
            g[i]=g[j];
        }
        else if(f[j]+Size[i]==f[i]){
            多了一条路径
            g[i]+=g[j];
        }
求最大值即可
*/
    for (int i = scc_cnt; i; i -- )
    {
        if (!f[i])
        {
            f[i] = scc_size[i];
            g[i] = 1;
        }
        for (int j = hs[i]; ~j; j = ne[j])
        {
            int k = e[j];
            if (f[k] < f[i] + scc_size[k])
            {
                f[k] = f[i] + scc_size[k];
                g[k] = g[i];
            }
            else if (f[k] == f[i] + scc_size[k])
                g[k] = (g[k] + g[i]) % mod;
        }
    }
    int maxf = 0, sum = 0;
    for (int i = 1; i <= scc_cnt; i ++ )
        if (f[i] > maxf)
        {
            maxf = f[i];
            sum = g[i];
        }
        else if (f[i] == maxf) sum = (sum + g[i]) % mod;

    printf("%d\n", maxf);
    printf("%d\n", sum);

    return 0;
}
